...
swift_release_bin_path() {
  local target swift_exe developer_dir sdkroot triple release_product
  target="$1"
  swift_exe="$(swift_bin)"
  ...
  (
    cd "$repo_root"
    for release_product in "${products[@]}"; do
      DEVELOPER_DIR="$developer_dir" SDKROOT="$sdkroot" \
        "$swift_exe" build -c release --product "$release_product" --triple "$triple" >/dev/null
    done
    DEVELOPER_DIR="$developer_dir" SDKROOT="$sdkroot" \
...
build_target() {
  local version target release_dir bin_path work_dir archive release_product
  version="$1"
  target="$2"
  release_dir="$3"
  work_dir="$release_dir/work/$artifact_name-$version-$target"
  ...
  for release_product in "${products[@]}"; do
    cp "$bin_path/$release_product" "$work_dir/bin/$release_product"
    chmod 0755 "$work_dir/bin/$release_product"
  done
